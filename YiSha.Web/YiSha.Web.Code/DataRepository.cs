    ...
    public class DataRepository : RepositoryFactory
    {
        public async Task<OperatorInfo> GetUserByToken(string token)
        {
            ...
            if (operatorInfo != null)
            {
                ...
                #region 门店
                strSql.Clear();
                strSql.Append(@"SELECT  a.BelongId as BranchId
                                FROM    SysUserBelong a
                                WHERE   a.UserId = " + operatorInfo.UserId + " AND ");
                strSql.Append("         a.BelongType = " + UserBelongTypeEnum.Branch.ParseToInt());
                IEnumerable<BranchInfo> branchList = await BaseRepository().FindList<BranchInfo>(strSql.ToString());
                operatorInfo.BranchIds = string.Join(",", branchList.Select(p => p.BranchId).ToArray());
                #endregion
