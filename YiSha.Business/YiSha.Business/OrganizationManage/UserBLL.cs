...
namespace YiSha.Business.OrganizationManage
{
    public class UserBLL
    {
        ...
        public async Task<TData<List<UserEntity>>> GetStewardList(long? branchId)
        {
            TData<List<UserEntity>> obj = new TData<List<UserEntity>>();
            List<UserEntity> userList = await userService.GetList(new UserListParam { });
            List<UserEntity> stewardList = new List<UserEntity>();
            PositionEntity position = await positionService.GetEntity("管家");
            foreach (var user in userList)
            {
                await GetUserBelong(user);
                if (!string.IsNullOrEmpty(user.PositionIds)&&user.PositionIds.Contains(position.Id.ToString()))
                {
                    if (branchId > 0)
                    {
                        if (!string.IsNullOrEmpty(user.BranchIds)&&user.BranchIds.Contains(branchId.ToString()))
                        {
                            stewardList.Add(user);
                        }
                    }
                    else
                    {
        ...
        private async Task GetUserBelong(UserEntity user)
        {
            ...
            List<UserBelongEntity> branchBelongList = userBelongList.Where(p => p.BelongType == UserBelongTypeEnum.Branch.ParseToInt()).ToList();
            if (branchBelongList.Count > 0)
            {
